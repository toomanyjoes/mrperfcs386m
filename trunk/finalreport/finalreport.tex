...
paper will present the results of our research on edge based cloud
computing and some of the benefits and challenges we discovered through
our work. In order to determine the full potential of this non-existent
network we use simulation and run MapReduce jobs that push the network
beyond the usual peer-to-peer/grid computing type simple data transfer
that these topologies are commonly used for. MapReduce operations
involve a high level of coordination and communication between machines
in addition to transferring large amounts of data. For our simulation
environment we investigate MRPerf, a MapReduce tool built on top of
...
based network we will change the parameters in our configuration to
