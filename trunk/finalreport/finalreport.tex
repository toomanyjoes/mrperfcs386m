...
country. \cite{Wang09MASCOTS} compares three types of data locality: node-local,
...
mapped Internet are presented in \ref{fig:numHosts}.
...
parameters the only variable being the specific property that was being tested.
...
We ran simulations that varied the number of nodes in a network for three different topologies.
...
The results from our experiments varying mappers and reducers per node (Figure \ref{fig:mapperReducer}) show that edge-based
...
We can conclude from our bandwidth experiments (Figure \ref{fig:bandwidth}) that adding additional
